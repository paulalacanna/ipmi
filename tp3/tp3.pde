...
int cantidadDeCirculos(float alto, float ancho) { //le tenes que dar las medidas que va a utilizar, en este caso el ancho y alto de la ventana como el codigo de abajo
  return int((alto / ancho) * 8 + 2);
}


void dibujarCentroConForAnidado(float posX, float posY) {
  int cantidad = cantidadDeCirculos(height, width); // se le esta diciendo que utilice el ancho y alto de la ventana
  //for anidado
